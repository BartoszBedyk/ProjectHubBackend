...
public class ActivityMapper {
	public static ActivityEntity toActivityEntity(Activity activity) {
		List<ActivityParamEntity> activityParamEntities =
				activity.getParams().stream().map(ActivityMapper::toActivityParamEntity).collect(Collectors.toList());
	...
	private static ActivityParamEntity toActivityParamEntity(ActivityParam activityParam) {
		return ActivityParamEntity.builder().paramName(activityParam.getName()).paramValue(activityParam.getValue()).build();
	}

	public static Activity toActivity(ActivityEntity activityEntity) {
		List<ActivityParam> activityParams =
				activityEntity.getParams().stream().map(ActivityMapper::toActivityParam).collect(Collectors.toList());
	...
	private static ActivityParam toActivityParam(ActivityParamEntity activityParamEntity) {
		return ActivityParam.builder().name(activityParamEntity.getParamName()).value(activityParamEntity.getParamValue()).build();
	}
}
